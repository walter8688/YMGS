...
{
    public class FootballGameListBL:FootballBLBase
    {
        public LanguageEnum Language { get; set; }

        public FootballGameListBL(){}

        public FootballGameListBL(LanguageEnum language)
        {
            this.Language = language;
        }

        public IList<FootballObject> GetFootballGameList()
        {
            ...
            if (dsMatchMarkets == null || dsMatchMarkets.Match_List.Count == 0)
                return null;
            var matchs = dsMatchMarkets.Match_List;
            ...
            foreach (var item in eventList)
            {
                ...
                fbObj.FootballMatchs = GetMatchs(events, matchs, item.EventId, item.MatchStartDate, markets, betBacks, betLays, defaultMarketTmpId);
                fbObjList.Add(fbObj);
            }
            #endregion
        ...
        private IList<FootballMatch> GetMatchs(DSEventTeamList.DSEventTeamListDataTable events, DSMatchAndMarket.Match_ListDataTable matchs, string eventId,
        ...
        {
            var matchList = from m in matchs
                            join e in events
