...
namespace YMGS.Business.GameSettle.SportMatchCalcRule
{
    ...
    /// 精确比分(波胆)
    /// </summary>
    public class PreciseScoreStrategy : IMatchSettelStrategy
    {
        /// <summary>
        /// 结算
        /// </summary>
            ...
            DSExchangeDealList.EXCHANGE_DEAL_LISTRow curExchangeDeal)
        {
            ...
            if (curExchangeDeal.MARKET_TMP_TYPE == (int)MarketTemplateTypeEnum.HalfTime)
                resultInfo.IsBuyerWin = (curMatch.HOME_FIR_HALF_SCORE == curExchangeDeal.MARKET_SCOREA &&
                    curMatch.GUEST_FIR_HALF_SCORE == curExchangeDeal.MARKET_SCOREB) ? 1 : 2;
            else
                resultInfo.IsBuyerWin = (curMatch.HOME_FULL_SCORE == curExchangeDeal.MARKET_SCOREA &&
                    curMatch.GUEST_FULL_SCORE == curExchangeDeal.MARKET_SCOREB) ? 1 : 2;
