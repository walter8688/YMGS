...
echo. >> fullTables.tmp
echo Create table scripts by name
rem File names are sorted by creation date
for /F %%f in ('dir "CreateScript\Table\*.sql" /B /O:N /A:-D-H-S') do (
	echo PRINT 'Create table %%f'
	copy /Y /B fullTables.tmp + "CreateScript\Table\%%f" fullTables.tmp || go errors
	echo. >> fullTables.tmp
)
copy /Y /B fullUpdateTemp.tmp + fullTables.tmp fullUpdateTemp.tmp || goto errors
del fullTables.tmp
...
rem ******************** Init data *********************
copy /Y /B fullUpdateTemp.tmp + "CreateScript\InitDefaultData.sql" fullUpdateTemp.tmp || go errors
