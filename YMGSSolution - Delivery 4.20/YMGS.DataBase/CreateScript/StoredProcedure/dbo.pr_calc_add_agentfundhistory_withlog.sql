IF EXISTS ( SELECT  *
            FROM    sysobjects
            WHERE   id = OBJECT_ID(N'[dbo].[pr_calc_add_agentfundhistory_withlog]')
                    AND OBJECTPROPERTY(id, N'Isprocedure') = 1 ) 
    DROP PROCEDURE [dbo].[pr_calc_add_agentfundhistory_withlog]
GO
CREATE PROCEDURE [dbo].[pr_calc_add_agentfundhistory_withlog]
    (
		@Log_Id int,
		@User_Fund_Id int,
		@Trade_Type int,
...
BEGIN
	...
	--用户资金历史记录日志的Log_type_Id=9
	exec pr_calc_log @Log_Id,9,@Come_User_Id,@Tmp_Identity_Id,@Trade_Type,@User_Fund_Id,@Trade_Fund,null,null,@Trade_Desc
END
GO